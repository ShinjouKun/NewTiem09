...
PipeLine::PipeLine()
{
	...
	//	descHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	//	descHeapDesc.NumDescriptors = TexSRVCount;
	//	DirectXManager::GetInstance()->Dev()->CreateDescriptorHeap(&descHeapDesc, IID_PPV_ARGS(&SpriteDescHeap));
	//}

	//if (!ParticleDescHeap)//初回のみ2D用
	//{
	//	//デスクリプタヒープを生成
	//	D3D12_DESCRIPTOR_HEAP_DESC descHeapDesc = {};
	//	descHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	//	descHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	//	descHeapDesc.NumDescriptors = TexSRVCount;
	//	DirectXManager::GetInstance()->Dev()->CreateDescriptorHeap(&descHeapDesc, IID_PPV_ARGS(&ParticleDescHeap));
	//}
}
...
//
//ID3D12DescriptorHeap * PipeLine::GetSpriteDescripterHeap() const
//{
